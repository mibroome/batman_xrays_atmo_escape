...
{ 
	...
	int nthreads;
	npy_intp dims[1];
	PyArrayObject *ds, *flux, *c1, *tau_np, *tau_np2;  

  	if(!PyArg_ParseTuple(args,"OdOddddddiOO", &ds, &rprs, &c1, &c2, &c3, &c4, &c5, &c6, &fac, &nthreads, &tau_np, &tau_np2)) return NULL; //parses input arguments
	...
	double *tau_arr2 = (double *)PyArray_DATA(tau_np2);
	...
	*/
	...
	calc_limb_darkening(f_array, d_array, dims[0], rprs, fac, nthreads, intVals, tau_arr, tau_arr2, tau_Nsteps);
