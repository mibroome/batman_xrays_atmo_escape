...
static PyObject *_power2_ld(PyObject *self, PyObject *args)
{
	...
	int nthreads;
	npy_intp dims[1];
	PyArrayObject *ds, *flux, *tau_np, *tau_np2;

  	if(!PyArg_ParseTuple(args,"OddddiOO", &ds, &rprs, &c1, &c2, &fac, &nthreads, &tau_np, &tau_np2)) return NULL; //parses input arguments
	...
	*/
	...
	calc_limb_darkening(f_array, d_array, dims[0], rprs, fac, nthreads, intensity_args, tau_arr, tau_arr2, tau_Nsteps);
