...
class TransitModel(object):
    ...
    def __init__(
        ...
        if (
            (params.limb_dark == "uniform" and len(params.u) != 0)
            or (params.limb_dark == "linear" and len(params.u) != 1)
            or (params.limb_dark == "quadratic" and len(params.u) != 2)
            or (params.limb_dark == "logarithmic" and len(params.u) != 2)
            or (params.limb_dark == "exponential" and len(params.u) != 2)
            or (params.limb_dark == "squareroot" and len(params.u) != 2)
            or (params.limb_dark == "power2" and len(params.u) != 2)
            or (params.limb_dark == "nonlinear" and len(params.u) != 4)
        ):
            raise Exception(
                "Incorrect number of coefficients for "
                + params.limb_dark
        ...
        if params.limb_dark not in [
            ...
            raise Exception(
                ...
                + params.limb_dark
                + '"'
                + " limb darkening not supported; allowed options are:\n \
				uniform, linear, quadratic, logarithmic, exponential, squareroot, nonlinear, power2, custom"
            )
        if max_err < 0.001:
            raise Exception(
                "The lowest allowed value for max_err is 0.001. For more accurate calculation, set the integration step size explicitly with the fac parameter."
            )
        if transittype not in ["primary", "secondary"]:
            raise Exception('Allowed transit types are "primary" and "secondary".')
        if supersample_factor > 1 and exp_time <= 0.0:
            raise Exception(
                "Please enter a valid exposure time (exp_time must be greater than 0 to calculate super-sampled light curves)."
            )
        if not isinstance(t, np.ndarray):
            raise Exception("Times t must be a numpy array (not a list).")
            ...
            if (
                ...
                if nthreads > multiprocessing.cpu_count():
                    raise Exception(
                        ...
                        + "{0:d}".format(multiprocessing.cpu_count())
                    )
                elif nthreads <= 1:
                    raise Exception(
                        "Number of threads must be between 2 and {0:d}".format(
                            multiprocessing.cpu_count()
                        )
                    )
                else:
                    raise Exception(
                        "OpenMP not enabled: do not set the nthreads parameter"
                    )
        self.ds = _rsky._rsky(
            self.t_supersample,
            params.t0,
    ...
    def calc_err(self, plot=False):
        ...
        if self.limb_dark in [
            ...
            ds = np.linspace(0.0, 1.1, 500)
            fac_lo = 5.0e-4
            if self.limb_dark == "nonlinear":
                f0 = _nonlinear_ld._nonlinear_ld(
                    ds,
                    self.rp,
                    self.u[0],
                    self.u[1],
                    ...
                    fac_lo,
                    self.nthreads,
                    np.ascontiguousarray(self.tau, dtype=np.float64),
                    np.ascontiguousarray(self.tau2, dtype=np.float64),
                    np.ascontiguousarray(self.xs, dtype=np.float64),
                )
                f = _nonlinear_ld._nonlinear_ld(
                    ds,
                    self.rp,
                    self.u[0],
                    self.u[1],
                ...
                f0 = _nonlinear_ld._nonlinear_ld(
                    ds, self.rp, self.u[1], self.u[0], 0.0, 0.0, fac_lo, self.nthreads,
                    np.ascontiguousarray(self.tau, dtype=np.float64),
                    np.ascontiguousarray(self.tau2, dtype=np.float64),
                    np.ascontiguousarray(self.xs, dtype=np.float64)
                )
                f = _nonlinear_ld._nonlinear_ld(
                    ds, self.rp, self.u[1], self.u[0], 0.0, 0.0, self.fac, self.nthreads,
                    np.ascontiguousarray(self.tau, dtype=np.float64),
                    np.ascontiguousarray(self.tau2, dtype=np.float64),
                    np.ascontiguousarray(self.xs, dtype=np.float64)
                )
            elif self.limb_dark == "exponential":
                f0 = _exponential_ld._exponential_ld(
                    ds, self.rp, self.u[0], self.u[1], fac_lo, self.nthreads,
                    np.ascontiguousarray(self.tau, dtype=np.float64),
                    np.ascontiguousarray(self.tau2, dtype=np.float64),
                    np.ascontiguousarray(self.xs, dtype=np.float64)
                )
                f = _exponential_ld._exponential_ld(
                    ds, self.rp, self.u[0], self.u[1], self.fac, self.nthreads,
                    np.ascontiguousarray(self.tau, dtype=np.float64),
                    np.ascontiguousarray(self.tau2, dtype=np.float64),
                    np.ascontiguousarray(self.xs, dtype=np.float64)
                )
            elif self.limb_dark == "logarithmic":
                f0 = _logarithmic_ld._logarithmic_ld(
                    ds, self.rp, self.u[0], self.u[1], fac_lo, self.nthreads,
                    np.ascontiguousarray(self.tau, dtype=np.float64),
                    np.ascontiguousarray(self.tau2, dtype=np.float64),
                    np.ascontiguousarray(self.xs, dtype=np.float64)
                )
                f = _logarithmic_ld._logarithmic_ld(
                    ds, self.rp, self.u[0], self.u[1], self.fac, self.nthreads,
                    np.ascontiguousarray(self.tau, dtype=np.float64),
                    np.ascontiguousarray(self.tau2, dtype=np.float64),
                    np.ascontiguousarray(self.xs, dtype=np.float64)
                ...
                f0 = _power2_ld._power2_ld(
                    ds, self.rp, self.u[0], self.u[1], fac_lo, self.nthreads,
                    np.ascontiguousarray(self.tau, dtype=np.float64),
                    np.ascontiguousarray(self.tau2, dtype=np.float64),
                    np.ascontiguousarray(self.xs, dtype=np.float64)
                )
                f = _power2_ld._power2_ld(
                    ds, self.rp, self.u[0], self.u[1], self.fac, self.nthreads,
                    np.ascontiguousarray(self.tau, dtype=np.float64),
                    np.ascontiguousarray(self.tau2, dtype=np.float64),
                    np.ascontiguousarray(self.xs, dtype=np.float64)
            ...
            raise Exception(
                "Function calc_err not valid for " + self.limb_dark + " limb darkening"
            )
    ...
    def _get_fac(self):
        if self.limb_dark in [
            ...
            nthreads = 1
            fac_lo, fac_hi = 5.0e-4, 1.0
            ds = np.linspace(0.0, 1.0 + self.rp, 1000)
            if self.limb_dark == "nonlinear":
                f0 = _nonlinear_ld._nonlinear_ld(
                    ds,
                    self.rp,
                ...
                f0 = _nonlinear_ld._nonlinear_ld(
                    ds, self.rp, self.u[1], self.u[0], 0.0, 0.0, fac_lo, nthreads,
                    np.ascontiguousarray(self.tau, dtype=np.float64),
                    np.ascontiguousarray(self.tau2, dtype=np.float64),
                    np.ascontiguousarray(self.xs, dtype=np.float64)
                )
                ...
                f0 = _exponential_ld._exponential_ld(
                    ds, self.rp, self.u[0], self.u[1], fac_lo, nthreads,
                    np.ascontiguousarray(self.tau, dtype=np.float64),
                    np.ascontiguousarray(self.tau2, dtype=np.float64),
                    np.ascontiguousarray(self.xs, dtype=np.float64)
                )
            ...
            while err > self.max_err or err < 0.99 * self.max_err:
                ...
                if self.limb_dark == "nonlinear":
                    ...
                    f = _nonlinear_ld._nonlinear_ld(
                        ds, self.rp, self.u[1], self.u[0], 0.0, 0.0, fac, nthreads,
                        np.ascontiguousarray(self.tau, dtype=np.float64),
                    np.ascontiguousarray(self.tau2, dtype=np.float64),
                    np.ascontiguousarray(self.xs, dtype=np.float64)
                    )
                    ...
                    f = _exponential_ld._exponential_ld(
                        ds, self.rp, self.u[0], self.u[1], fac, nthreads,
                    np.ascontiguousarray(self.tau, dtype=np.float64),
                    np.ascontiguousarray(self.tau2, dtype=np.float64),
                    np.ascontiguousarray(self.xs, dtype=np.float64)
                    )
    ...
    def light_curve(self, params):
        ...
        if self.transittype == 1:
            if params.limb_dark != self.limb_dark:
                raise Exception(
                    "Need to reinitialize model in order to change limb darkening option"
                )
            if self.limb_dark == "quadratic":
                lc = _quadratic_ld._quadratic_ld(
                    self.ds, np.abs(params.rp), params.u[0], params.u[1], self.nthreads
                )
            elif self.limb_dark == "linear":
                lc = _quadratic_ld._quadratic_ld(
                    self.ds, np.abs(params.rp), params.u[0], 0.0, self.nthreads
                )
            elif self.limb_dark == "nonlinear":
                lc = _nonlinear_ld._nonlinear_ld(
                    self.ds,
                    np.abs(params.rp),
                    params.u[0],
                    params.u[1],
                    ...
                    self.fac,
                    self.nthreads,
                    np.ascontiguousarray(params.tau, dtype=np.float64),
                    np.ascontiguousarray(params.tau2, dtype=np.float64),
                    np.ascontiguousarray(self.xs, dtype=np.float64),
                )
            elif self.limb_dark == "squareroot":
                lc = _nonlinear_ld._nonlinear_ld(
                ...
                lc = _uniform_ld._uniform_ld(self.ds, np.abs(params.rp), self.nthreads)
            elif self.limb_dark == "logarithmic":
                lc = _logarithmic_ld._logarithmic_ld(
                    self.ds,
                    np.abs(params.rp),
                    params.u[0],
                    params.u[1],
                ...
                lc = _exponential_ld._exponential_ld(
                    self.ds,
                    np.abs(params.rp),
                    params.u[0],
                    params.u[1],
            ...
            elif self.limb_dark == "custom":
                lc = _custom_ld._custom_ld(
                    self.ds,
                    np.abs(params.rp),
                    params.u[0],
                    params.u[1],
            ...
            lc = _eclipse._eclipse(self.ds, np.abs(params.rp), params.fp, self.nthreads)
    ...
    def _get_phase(self, params, position):
        ...
        E = 2.0 * np.arctan(
            np.sqrt((1.0 - params.ecc) / (1.0 + params.ecc)) * np.tan(TA / 2.0)
        )
...
class TransitParams(object):
    ...
    def __init__(self):
        ...
        self.tau = None  # array of slant-path optical depths (length = number of radial steps)
        self.tau2 = None  # second array of slant-path optical depths
