...
static PyObject *_rsky_x(PyObject *self, PyObject *args);
...
{
	/*
		Computes the signed sky-plane x coordinate of the planet centre:
		    x_sky = r * cos(f + omega)
		where r = a*(1-e^2)/(1+e*cos(f)) is the orbital radius.
		This quantity is positive during ingress and negative during egress
		for a standard prograde orbit, providing the directional information
		lost by the scalar distance returned by _rsky.
	*/
	double ecc, inc, a, omega, per, tc, BIGD = 100.;
	int transittype, nthreads;
	npy_intp dims[1];
	...
	dims[0] = PyArray_DIMS(ts)[0];
	xs = (PyArrayObject *) PyArray_SimpleNew(1, dims, PyArray_TYPE(ts));

	double *t_array = PyArray_DATA(ts);
	double *output_array = PyArray_DATA(xs);

	const double n = 2.*M_PI/per;
	...
	for(int i = 0; i < dims[0]; i++)
	{
		...
		double tp = tc - per*M/2./M_PI;

		if(ecc < 1.0e-5)
		{
			f = ((t - tp)/per - (int)((t - tp)/per))*2.*M_PI;
		}
		else
		{
			M = n*(t - tp);
		...
		output_array[i] = r * cos(f + omega);  /* positive = ingress, negative = egress */
	}
	return PyArray_Return((PyArrayObject *)xs);
}


static char _rsky_x_doc[] = "Computes the signed sky-plane x coordinate r*cos(f+omega). Positive during ingress, negative during egress.";
...
static PyMethodDef _rsky_methods[] = {
  {"_rsky", _rsky,METH_VARARGS,_rsky_doc},{"_getf", _getf,METH_VARARGS,_getf_doc},{"_rsky_x", _rsky_x,METH_VARARGS,_rsky_x_doc},{NULL}};
