...
double area(double d, double x, double R, double *tau_arr, double *tau_arr2, int Nsteps)
{
	...
	double theta_blob = 1.05; //radians, from experimenting: 1Z = 1.05 rad, 
	double dr = R / Nsteps;
	...
	if(x <= R - d) return M_PI*x*x;						//stellar circle completely overlaps planet
		...
		for (int ir = 0; ir < Nsteps; ++ir) {
			...
			blocked_flux += transmission * 2.0 * M_PI * r_n * dr * (2*M_PI-2*theta_blob)/ (2*M_PI); //this is the contribution from the portion of the annulus outside of the blob
			
			/*Contribution from within blob*/
			transmission = 1.0 - exp(-tau_arr2[ir]);
			if (transmission < 0) transmission = 0.0;
			if (transmission > 1.0) transmission = 1.0;
			blocked_flux += transmission * 2.0 * M_PI * r_n * dr * (2*theta_blob)/ (2*M_PI); //this is the contribution from the portion of the annulus outside of the blob
		}
		return blocked_flux;
	}
		...
		for (int ir = 0; ir < Nsteps; ++ir) {
			...
			//This simplified scheme ONLY WORKS FOR INCLINATIONS NEAR 90 DEGREES
			double transmission;
...
void calc_limb_darkening(double* f_array, double* d_array, int N, double rprs, double fac, int nthreads, double *intVals, double *tau_arr, int tau_Nsteps, double *tau2_arr, int tau2_Nsteps)
{
	/*
	...
	*/
	...
	for(int i = 0; i < N; i++)
	{
		...
		else
		{
			...
			while(x < x_out)
			{
				double A_f = area(d, x, rprs, tau_arr,tau2_arr, tau_Nsteps );				//calculates area of overlapping circles
			...
			double A_f = area(d, x, rprs, tau_arr,  tau2_arr, tau_Nsteps);					//area for last integration step
			double I = intensity(x - dx/2., intVals); 		//intensity at the midpoint
			delta += (A_f - A_i)*I;					//increase in transit depth for this integration step
