...
static PyObject *_power2_ld(PyObject *self, PyObject *args)
{
	...
	int nthreads;
	npy_intp dims[1];
	PyArrayObject *ds, *flux, *tau_np, *tau2_np;

  	if(!PyArg_ParseTuple(args,"OddddiOO", &ds, &rprs, &c1, &c2, &fac, &nthreads, &tau_np, &tau2_np)) return NULL; //parses input arguments
	...
	*/
	...
	calc_limb_darkening(f_array, d_array, dims[0], rprs, fac, nthreads, intensity_args, tau_arr, tau_Nsteps, tau2_arr, tau2_Nsteps);
