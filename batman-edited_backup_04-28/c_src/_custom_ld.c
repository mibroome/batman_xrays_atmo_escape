...
{ 
	...
	int nthreads;
	npy_intp dims[1];
	PyArrayObject *ds, *flux, *c1, *tau_np, *tau2_np;  

  	if(!PyArg_ParseTuple(args,"OdOddddddiOO", &ds, &rprs, &c1, &c2, &c3, &c4, &c5, &c6, &fac, &nthreads, &tau_np, &tau2_np)) return NULL; //parses input arguments
	...
	*/
	...
	calc_limb_darkening(f_array, d_array, dims[0], rprs, fac, nthreads, intVals, tau_arr, tau_Nsteps, tau2_arr, tau2_Nsteps);
